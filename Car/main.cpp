...
public:
	...
	void control()
	{
		...
		{
			key = 0;
			if(_kbhit())key = _getch(); //Ожидает нажатия клавиши и возвращает ASCII-код нажатой клавиши.
			switch (key)
			{
			case Enter:
	...
	void panel()const
	{
		while (driver_inside)
		{
			...
			cout << "Consumption per second:\t" << engine.get_consumtion_per_second() << " liters.\n";
...
void main()
{
	...
	do
	{
		cout << "Введите обхем топлива: "; cin >> fuel;
		tank.fill(fuel);
		tank.info();
	} while (fuel > 0);
#endif // TANK_CHECK
